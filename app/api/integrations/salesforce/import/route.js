...
/**
 * Preview Salesforce contacts (first page).
 */
export async function GET(request) {
  ...
  if (!integration || integration.status !== "connected") {
    return NextResponse.json({ error: "Connect Salesforce first." }, { status: 400 });
  }

  try {
    const accessToken = await getValidAccessToken(session.supabase, integration);
    const { instanceUrl } = getSalesforceConfig(integration);
    if (!instanceUrl) throw new Error("Salesforce instance URL missing. Reconnect.");

    const nextUrl = new URL(request.url).searchParams.get("next") || undefined;
    const data = await listContacts(accessToken, instanceUrl, {
      nextUrl: nextUrl || undefined,
      limit: 50,
    });

    const contacts = (data.records || []).map((c) => {
      const mapped = mapSalesforceContactToLead(c);
      return {
...
/**
 * Import selected (or first N) Salesforce contacts as leads.
 */
export async function POST(request) {
  ...
  const integration = await getSalesforceIntegration(session.supabase, workspaceId);
  if (!integration || integration.status !== "connected") {
    return NextResponse.json({ error: "Connect Salesforce first." }, { status: 400 });
  }
  ...
  try {
    ...
    /** @type {Array<ReturnType<typeof mapSalesforceContactToLead>>} */
    const toImport = [];
    let nextUrl;
    let pages = 0;

    while (toImport.length < max && pages < 10) {
      const page = await listContacts(accessToken, instanceUrl, {
        nextUrl,
        limit: 50,
      });
      pages += 1;
      for (const contact of page.records || []) {
        const mapped = mapSalesforceContactToLead(contact);
      ...
      if (!nextUrl) break;
      if (wanted && toImport.length >= wanted.size) break;
    }
    ...
    for (const item of toImport) {
      ...
      const { error: linkError } = await session.supabase.from("lead_external_links").insert({
        ...
        external_url: `${instanceUrl}/${item.externalId}`,
      });
