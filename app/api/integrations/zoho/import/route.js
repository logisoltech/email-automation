...
/**
 * Preview Zoho contacts (first page).
 */
export async function GET(request) {
  ...
  if (!integration || integration.status !== "connected") {
    return NextResponse.json({ error: "Connect Zoho first." }, { status: 400 });
  }

  try {
    const accessToken = await getValidAccessToken(session.supabase, integration);
    const { apiDomain } = getZohoConfig(integration);
    const page = Number(new URL(request.url).searchParams.get("page") || 1);
    const data = await listContacts(accessToken, apiDomain, { page, perPage: 50 });
    const contacts = (data.data || []).map((c) => {
      const mapped = mapZohoContactToLead(c);
      return {
...
/**
 * Import selected (or first N) Zoho contacts as leads.
 */
export async function POST(request) {
  ...
  const integration = await getZohoIntegration(session.supabase, workspaceId);
  if (!integration || integration.status !== "connected") {
    return NextResponse.json({ error: "Connect Zoho first." }, { status: 400 });
  }
  ...
  try {
    ...
    /** @type {Array<ReturnType<typeof mapZohoContactToLead>>} */
    const toImport = [];
    let page = 1;
    let more = true;

    while (toImport.length < max && more) {
      const result = await listContacts(accessToken, apiDomain, { page, perPage: 50 });
      const rows = result.data || [];
      for (const contact of rows) {
        const mapped = mapZohoContactToLead(contact);
        if (!mapped.email) continue;
        if (wanted && !wanted.has(mapped.externalId)) continue;
        toImport.push(mapped);
        if (toImport.length >= max) break;
      }
      more = Boolean(result.info?.more_records);
      page += 1;
